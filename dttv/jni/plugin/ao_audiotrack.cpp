...
static int android_audio_get_volume(ao_wrapper_t *ao)
{
    return 0;
}

static int android_audio_set_volume(ao_wrapper_t *ao, int value)
{
    return 0;
}

const char *ao_audiotrack_name = "AUDIOTRACK AO";

void ao_audiotrack_setup(ao_wrapper_t *ao)
{
	ao->id = 0x100;//AO_ID_ANDROID
    ao->name = ao_audiotrack_name;
    ao->ao_init = android_audio_init;
    ao->ao_pause = android_audio_pause;
    ao->ao_resume = android_audio_resume;
    ao->ao_stop = android_audio_stop;
    ao->ao_write = android_audio_play;
    ao->ao_level = android_audio_level;
    ao->ao_latency = android_audio_latency;
    ao->ao_get_volume = android_audio_get_volume;
    ao->ao_set_volume = android_audio_set_volume;
}
} // end extern  "C"
